...
var TICK_KZ = ["Арматура","Профильді құбыр","СГҚ және э/д құбыр","Бұрыштама","Швеллер","Арқалық","Табақ","Жолақ","Катанка","Сым"];
...
var CALC = [
{id:"armatura", n:"Арматура", nk:"Арматура", d:5, rows:[
...
{id:"truba-profilnaya", n:"Труба профильная", nk:"Профильді құбыр", rows:[
...
{id:"truba-kruglaya", n:"Труба круглая", nk:"Дөңгелек құбыр", rows:[
...
{id:"ugolok", n:"Уголок", nk:"Бұрыштама", rows:[
...
{id:"shveller", n:"Швеллер", nk:"Швеллер", rows:[
...
{id:"balka", n:"Балка двутавровая", nk:"Қоставр арқалық", rows:[
...
{id:"list", n:"Лист стальной", nk:"Болат табақ", rows:[
...
{id:"polosa-katanka", n:"Полоса, катанка, проволока", nk:"Жолақ, катанка, сым", rows:[
...
/* казахские подписи размеров: подменяем термины прямо в строке */
var KZ_SIZE = [[/бухта/g,"орам"],[/пруток/g,"шыбық"],[/Полоса/g,"Жолақ"],[/Проволока/g,"Сым"],[/оцинк\./g,"мырышт."],[/Рифлёный/g,"Бедерлі"],[/Г\/к/g,"Ы/и"],[/Х\/к/g,"С/и"],[/ВГП/g,"СГҚ"],[/Э\/с/g,"Э/д"]];
var UNIT = {ru:{pc:"шт",coil:"бухт",sheet:"лист",m:"м",t:"т"}, kk:{pc:"дана",coil:"орам",sheet:"табақ",m:"м",t:"т"}};
...
function sizeLabel(row){
  var s = row.s;
  if (curLang() === "kk") KZ_SIZE.forEach(function(r){ s = s.replace(r[0], r[1]); });
  return s;
}
function calcLang(){
  if (!kCat) return;
  ...
  var kk = curLang() === "kk";
  kCat.innerHTML = CALC.map(function(c){ return "<option value=\"" + c.id + "\">" + (kk ? c.nk : c.n) + "</option>"; }).join("");
  kCat.selectedIndex = ci;
  fillSizes(si);
  if (zCat) {
    var zi = zCat.selectedIndex < 0 ? 0 : zCat.selectedIndex;
    zCat.innerHTML = CALC.map(function(c){ return "<option value=\"" + c.id + "\">" + (kk ? c.nk : c.n) + "</option>"; }).join("") +
      "<option value=\"other\">" + (kk ? "Басқа / бірнеше позиция" : "Другое / несколько позиций") + "</option>";
...
function calc(){
  ...
  var sum = t * r.p;
  var pu = r.u || "pc";
  document.getElementById("k-title").textContent = (L === "kk" ? c.nk : c.n) + " " + sizeLabel(r);
  ...
  var line = (L === "kk" ? "Есеп:\n" : "Расчёт:\n") + (L === "kk" ? c.nk : c.n) + " " + sizeLabel(r) + "\n" +
    fmt(q, q % 1 ? 2 : 0) + " " + unitTxt + " ≈ " + (r.l ? smart(m) + " " + U.m + " / " : "") + smart(t) + " " + U.t + " / " + smart(pcs) + " " + U[pu] + "\n" +
    (L === "kk" ? "Прайс бойынша болжамды: " : "Ориентировочно по прайсу: ") + fmt(sum, 0) + " ₸\n" +
    (L === "kk" ? "Бағасын, қолда барын және жеткізуді растаңызшы." : "Прошу подтвердить цену, наличие и доставку.");
  var wa = document.getElementById("k-wa");
  if (wa) wa.href = "https://wa.me/" + WA + "?text=" + encodeURIComponent((L === "kk" ? "Сәлеметсіз бе! Strong Steel сайтынан. " : "Здравствуйте! С сайта Strong Steel. ") + line);
}
...
if (form) form.addEventListener("submit", function(e){
  ...
  var L = curLang();
  var t = (L === "kk"
    ? "Сәлеметсіз бе! Strong Steel сайтынан өтінім.\nАты: " + name + "\nТелефон: " + phone + "\nНе керек: " + catTxt + (vol ? "\nКөлем: " + vol : "") + (msg ? "\nТүсініктеме: " + msg : "")
    : "Здравствуйте! Заявка с сайта Strong Steel.\nИмя: " + name + "\nТелефон: " + phone + "\nЧто нужно: " + catTxt + (vol ? "\nОбъём: " + vol : "") + (msg ? "\nКомментарий: " + msg : ""));
  ok.hidden = false;
  conv("lead");
